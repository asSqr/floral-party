...
LOGO_CARD_IMAGE_PATH = './public/static/images/logo_card.png'
LOGO_TRANS_IMAGE_PATH = './public/static/images/logo_trans.png'
...
def hconcat_resize(image_list, interpolation=cv2.INTER_CUBIC, h_min=600):
    image_list_resize = [
...
def vconcat_resize(image_list, interpolation=cv2.INTER_CUBIC, w_min=1200):
    image_list_resize = [
...
logo_trans_image = cv2.imread(LOGO_TRANS_IMAGE_PATH, cv2.IMREAD_UNCHANGED)

logo_width = result_image.shape[1] // 4 * 3
logo_height = result_image.shape[0] // 4 * 3
...
logo_trans_image = cv2.resize(logo_trans_image, (logo_width, logo_height), interpolation=cv2.INTER_CUBIC)
...
result_image[y1:y2, x1:x2] = (result_image[y1:y2, x1:x2] * (1 - alpha) + \
    logo_card_image[y1-margin_height:y2-margin_height, x1-margin_width:x2-margin_width] * alpha) * (1-logo_trans_image[y1-margin_height:y2-margin_height, x1-margin_width:x2-margin_width, 3:] / 255) + \
    logo_trans_image[y1-margin_height:y2-margin_height, x1-margin_width:x2-margin_width, :3]  * (logo_trans_image[y1-margin_height:y2-margin_height, x1-margin_width:x2-margin_width, 3:] / 255)
